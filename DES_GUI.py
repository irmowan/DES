...
class Application(Frame):
    def __init__(self, master=None):
        ...
        self.label1 = Label(self, text='Text(Plain/Cipher)')
        self.label2 = Label(self, text='Key(64-bit)')
        self.label3 = Label(self, text='Result(Cipher/Plain)')
        self.textText = Text(self, width=60, height=5)
        self.keyText = Text(self, width=60, height=5)
        self.resultText = Text(self, width=60, height=5)
        self.textFileButton = Button(self, text='Open', command=self.open_text_file)
        self.keyFileButton = Button(self, text='Open', command=self.open_key_file)
        self.resultFileButton = Button(self, text='Save', command=self.save_result_file)
    ...
    def create_widgets(self):
        ...
        self.textText.grid(column=1, row=0, columnspan=2)
        self.keyText.grid(column=1, row=1, columnspan=2)
        self.resultText.grid(column=1, row=2, columnspan=2)

        self.textFileButton.grid(column=3, row=0)
        self.keyFileButton.grid(column=3, row=1)
        self.resultFileButton.grid(column=3, row=2)

        self.encryptButton.grid(column=1, row=4)
        self.decryptButton.grid(column=2, row=4)

    def open_text_file(self):
        self.inputText = filedialog.askopenfile('r').read().strip()
        self.textText.delete(0.0, END)
        self.textText.insert(INSERT, self.inputText)

    def open_key_file(self):
        self.inputKey = filedialog.askopenfile('r').read().strip()
        self.keyText.delete(0.0, END)
        self.keyText.insert(INSERT, self.inputKey)

    def save_result_file(self):
        self.outputResult = self.resultText.get('1.0', END)
        self.save_file_name = filedialog.askopenfile('r').name
        self.save_file = open(self.save_file_name, 'w')
        self.save_file.write(self.outputResult)
        self.save_file.close()

    def encrypt(self):
        self.text = self.textText.get(1.0, END)
        self.key = self.keyText.get(1.0, END)
        self.resultAnswer = main(self.text, self.key, 1)
        self.resultText.delete(0.0, END)
        self.resultText.insert(INSERT, self.resultAnswer)

    def decrypt(self):
        self.text = self.textText.get('1.0', END)
        self.key = self.keyText.get('1.0', END).strip()
        self.resultAnswer = main(self.text, self.key, 2)
        self.resultText.delete(0.0, END)
        self.resultText.insert(INSERT, self.resultAnswer)


app = Application()
app.master.title('DES')
app.mainloop()
